...
export class Game extends EventEmitter {
    ...
    async graphic(board, player1, player2) {
        ui.draw(board);
        await tf.nextFrame();
    }

    async start_human_play(player1, player2, start_player = 0, is_shown = 1) {
        ...
        if (is_shown) {
            await this.graphic(this.board, player1.player, player2.player);
        }

        while (true) {
            let current_player = this.board.get_current_player();
            let player_in_turn = players[current_player];
            let move = await player_in_turn.get_action(this.board);
    ...
    async start_play(player1, player2, start_player = 0, is_shown = 1) {
        if ([0, 1].indexOf(start_player) === -1) {
            throw new Error(`start_player should be either 0 (player1 first) or 1 (player2 first)`);
        }
        this.board.init_board(start_player);
    ...
    async start_self_play(player, temp = 1e-3, is_shown = 0) {
        ...
        while (1) {
            ...
            if (is_shown) {
                await this.graphic(this.board, p1, p2);
            }
