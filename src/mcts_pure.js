...
export default class MCTSPlayer {
...
class MCTS {
    ...
    _evaluate_rollout(state, limit = 1000) {
        ...
        for (let i of _.range(limit)) {
            ...
            let max_action = action_probs[Math.floor(Math.random() * action_probs.length)];
...
function rollout_policy_fn(board) {
    let action_probs = board.availables;
    return action_probs;
}
